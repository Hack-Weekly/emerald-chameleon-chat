...
namespace EmeraldChameleonChat.Services.DAL.DbContexts
{
    public class EmeraldChameleonChatContext : DbContext //must implement DbContext
    {
        ...
        public DbSet<WeatherForecast> WeatherForecast { get; set; } = null!; //must create a DbSet for each Entity. DbSets are used to query & save data to the db. Linq queries against a dbset translate to queries against the db.
        public DbSet<User> Users { get; set; }
        public DbSet<ChatRoomMessage> ChatRoomMessage { get; set; } = null!; //must create a DbSet for each Entity. DbSets are used to query & save data to the db. Linq queries against a dbset translate to queries against the db.
        public DbSet<ChatRoom> Chatroom { get; set; } = null!; //must create a DbSet for each Entity. DbSets are used to query & save data to the db. Linq queries against a dbset translate to queries against the db.
