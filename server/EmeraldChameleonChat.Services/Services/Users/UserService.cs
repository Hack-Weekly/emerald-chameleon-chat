...
namespace EmeraldChameleonChat.Services.Services.Users
{
    public class UserService : IUserService
    {
        ...
        public const string ConfirmationURL = "https://localhost:5001/api/auth/Confirmation";

        public UserService(IUserRepository registeredUserRepository, IAuthenticationService authenticationService,
                           IMapper mapper, IWebHostEnvironment webHostEnvironment, IEmailService emailService)
        {
        ...
        public async Task SendConfirmation(User registeredUser)
        {
            try
            {
                ...
                body = body.Replace("{ConfirmationURL}", $"{ConfirmationURL}?confirmationCode={registeredUser.ConfirmationCode}&userName={registeredUser.Username}");
